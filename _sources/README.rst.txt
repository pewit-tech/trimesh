...
|Build Status| |Build status| |Coverage Status| |PyPI version| |Build
Status| |Join the chat at https://gitter.im/trimsh/Lobby|
...
For the easiest install with *only* these minimal dependencies ``pip``
can generally install ``trimesh`` cleanly on Windows, Linux, and OSX:
...
-  Automatic hashing of numpy arrays storing key data for change
   tracking using MD5, zlib CRC, or xxhash
...
-  Calculate mass properties, including volume, center of mass, moment
   of inertia, and principal components of inertia vectors and
   components
-  Repair triangle winding and normals to be consistent
-  Convex hulls of meshes
-  Compute an identifier that is mostly
   rotation/translation/tessellation invariant
...
-  ``q`` closes the window
-  ``a`` toggles an XYZ-RGB axis marker between three states: off, at
   world frame, or at every frame
...
.. |Build status| image:: https://ci.appveyor.com/api/projects/status/j8h3luwvst1tkghl/branch/master?svg=true
   :target: https://ci.appveyor.com/project/mikedh/trimesh/branch/master
...
.. |Build Status| image:: https://dev.azure.com/mikedh0531/trimesh/_apis/build/status/mikedh.trimesh?branchName=master
   :target: https://dev.azure.com/mikedh0531/trimesh/_build/latest?definitionId=1&branchName=master
